 ...
 */
static void
write_caminfo(void *shm_caminfo, const std::vector<ClientPublisher> &clients,
              const std::vector<sl::FusionConfiguration> &configurations,
              const std::vector<Eigen::Matrix4d> &T_cams_extrinsics, int width,
              int height) {
  auto *hdr = shm_caminfo_header(shm_caminfo);
  hdr->ready.store(0,
                   std::memory_order_release); // mark not-ready while writing
  ...
  for (int i = 0; i < static_cast<int>(clients.size()); ++i) {
    ...
    // T_cams_extrinsics[i] is already in ROS world frame
    ...
    std::cout << "[SHM] cam " << i << " SN=" << e->serial_number
              << " fx=" << e->fx << " fy=" << e->fy << " cx=" << e->cx
              << " cy=" << e->cy << "\n"
...
int main(int argc, char **argv) {
  ...
  std::string calib_file = cfg.get_str("calibration_file", "ZED_CALIB_FILE",
                                       "two_cams_fusion_config.json");
  std::string yolo_model_path =
      cfg.get_str("yolo_model_path", "ZED_YOLO_PATH", "yolov8s-seg.onnx");
  std::string resolution_str =
      cfg.get_str("resolution", "ZED_RESOLUTION", "1280x720");
  double voxel_size = cfg.get_double("published_body_filter_voxel_size",
                                     "ZED_VOXEL_SIZE", 0.015);
  int erode_kernel =
      cfg.get_int("erode_body_mask_kernel_size", "ZED_ERODE_KERNEL", 18);
  bool overlay_yolo_mask =
      cfg.get_bool("overlay_yolo_mask", "ZED_OVERLAY_YOLO", true);
  ...
  for (auto &conf : configurations)
    write_caminfo(shm_caminfo, clients, configurations, T_cams_extrinsics,
                  width, height);
      ...
      for (size_t i = 0; i < clients.size(); ++i) {
        ...
        if (clients[i].zed.retrieveImage(zed_image, sl::VIEW::LEFT) ==
          ...
          if (overlay_yolo_mask && !yolo_model_path.empty())
            displayed =
                clients[i].overlayPersonMask(cv_img, human_masks[i], human_bboxes[i]);
