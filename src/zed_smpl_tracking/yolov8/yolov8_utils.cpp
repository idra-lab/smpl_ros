...
bool CheckParams(int netHeight, int netWidth, const int* netStride,
  ...
  if (netHeight % netStride[strideSize - 1] != 0 ||
    ...
    std::cout
        << "Error:_netHeight and _netWidth must be multiple of max stride "
        << netStride[strideSize - 1] << "!" << std::endl;
    return false;
  }
  return true;
}
...
void GetMask(const cv::Mat& maskProposals, const cv::Mat& maskProtos,
  ...
  cv::Mat protos =
      maskProtos.reshape(0, {seg_channels, seg_width * seg_height});
...
int BBox2Obb(float centerX, float centerY, float boxW, float boxH, float angle,
  ...
  rotatedRect = cv::RotatedRect(cv::Point2f(centerX, centerY),
                                cv::Size2f(boxW, boxH), angle);
  return 0;
}
...
void DrawPred(cv::Mat& img, std::vector<OutputParams> result,
              ...
              std::vector<cv::Scalar> color, bool isVideo) {
  ...
  for (int i = 0; i < result.size(); i++) {
    ...
    if (result[i].rotatedBox.size.width * result[i].rotatedBox.size.height >
        0) {
    ...
    std::string label =
        classNames[result[i].id] + ":" + std::to_string(result[i].confidence);
    int baseLine;
    cv::Size labelSize =
        cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine);
    top = MAX(top, labelSize.height);
    // rectangle(frame, cv::Point(left, top - int(1.5 * labelSize.height)),
    // cv::Point(left + int(1.5 * labelSize.width), top + baseLine),
    // cv::Scalar(0, 255, 0), FILLED);
    putText(img, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 1,
            color[result[i].id], 2);
  }
  cv::addWeighted(img, 0.5, mask, 0.5, 0, img);  // add mask to src
  cv::imshow("1", img);
  if (!isVideo) cv::waitKey();
  // destroyAllWindows();
}

void DrawPredPose(cv::Mat& img, std::vector<OutputParams> result,
                  PoseParams& poseParams, bool isVideo) {
  for (int i = 0; i < result.size(); i++) {
    int left, top;
    int color_num = i;
    if (result[i].box.area() > 0) {
    ...
    // cv::Scalar(0, 255, 0), FILLED);
    putText(img, label, cv::Point(left, top), cv::FONT_HERSHEY_SIMPLEX, 1,
            poseParams.personColor, 2);
    if (result[i].keyPoints.size() != poseParams.kptBodyNames.size()) continue;
    for (int j = 0; j < result[i].keyPoints.size(); ++j) {
      PoseKeyPoint kpt = result[i].keyPoints[j];
      if (kpt.confidence < poseParams.kptThreshold) continue;
      cv::Scalar kptColor = poseParams.posePalette[poseParams.kptColor[j]];
      cv::circle(img, cv::Point(kpt.x, kpt.y), poseParams.kptRadius, kptColor,
                 -1, 8);
    }
    if (poseParams.isDrawKptLine) {
      for (int j = 0; j < poseParams.skeleton.size(); ++j) {
        ...
        if (kpt0.confidence < poseParams.kptThreshold ||
            kpt1.confidence < poseParams.kptThreshold)
          continue;
        cv::Scalar kptColor = poseParams.posePalette[poseParams.limbColor[j]];
        cv::line(img, cv::Point(kpt0.x, kpt0.y), cv::Point(kpt1.x, kpt1.y),
                 kptColor, 2, 8);
      }
    }
  }
